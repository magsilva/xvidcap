...
init (XVC_CapTypeOptions * ctos, int *argc, char *argv[])
{
    ...
    if (!xvc_init_pre (*argc, argv)) {
        fprintf (stderr,
                 _("Can't do GUI pre-initialization ... aborting\n"));
        exit (2);
    }
    //
...
main (int argc, char *argv[])
{
    ...
    // get errors in options to pass to xvc_ui_create
    {
        ...
        if (rc == -1) {
            fprintf (stderr,
                     _
                     ("Unrecoverable error while validating options, please contact the xvidcap project.\n"));
            exit (1);
        }
    }
    ...
    if (!xvc_ui_create ()) {
        fprintf (stderr, _("Can't create GUI ... aborting\n"));
        exit (2);
    }
    if (!xvc_frame_create (capture_window)) {
        fprintf (stderr,
                 _("Can't create selection Frame ... aborting\n"));
        exit (2);
    }
    if (!xvc_ui_init (errors_after_cli)) {
        fprintf (stderr, _("Can't initialize GUI ... aborting\n"));
        exit (2);
    }
