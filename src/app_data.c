 ...
 */
static XVC_ErrorListItem *errorlist_append (int code, XVC_ErrorListItem * err, XVC_AppData * app);
...
xvc_appdata_set_defaults (XVC_AppData * lapp)
{
    ...
    lapp->single_frame.fps = lapp->multi_frame.fps = (XVC_Fps) {10, 1};
    ...
    lapp->multi_frame.edit_cmd = _("xterm -e \"echo none specified; echo hit enter to dismiss; read\"");
    lapp->multi_frame.video_cmd = _("xterm -e \"echo not needed for multi-frame capture; echo hit enter to dismiss; read\"");
...
appdata_set_display (XVC_AppData * lapp)
{
    ...
    if (!lapp->dpy) {
        ...
        snprintf (msg, 256, "Can't get display to capture from!\n");
        perror (msg);
        return;
    }
